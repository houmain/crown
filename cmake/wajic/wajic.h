...
#define WAJIC(ret, name, args, ...) WA_EXTERN __attribute__((import_module("J"), import_name(#name DC1 #args DC1 #__VA_ARGS__))) ret name args;

// Macro to generate a JavaScript function that can be called from C with additional shared init code
#define WAJIC_WITH_INIT(INIT, ret, name, args, ...) WA_EXTERN __attribute__((import_module("J"), import_name(#name DC1 #args DC1 #__VA_ARGS__ DC1 DC1 #INIT ))) ret name args;

// Macro to generate a JavaScript function sharing same init code that can be called from C
#define WAJIC_LIB(lib, ret, name, args, ...) WA_EXTERN __attribute__((import_module("J"), import_name(#name DC1 #args DC1 #__VA_ARGS__ DC1 #lib))) ret name args;

// Macro to generate a JavaScript function that can be called from C also specifying shared init code
#define WAJIC_LIB_WITH_INIT(lib, INIT, ret, name, args, ...) WA_EXTERN __attribute__((import_module("J"), import_name(#name DC1 #args DC1 #__VA_ARGS__ DC1 #lib DC1 #INIT))) ret name args;
